...
// TODO: add code here
var header = document.getElementById("header"); //find header
var p = header.getElementsByTagName("p"); 
var ople = p[0].textContent
ApplePie.creator = ople
console.log(ApplePie.creator)

// loop over all ingredients 
var ingredient = document.getElementsByTagName("ul")[0]
for(var i = 0; i < (ingredient.getElementsByTagName("li").length);i++) {
	ApplePie.ingredients.push(ingredient.getElementsByTagName("li")[i].textContent)
} 
...
// document.addEventListener("click", write);
 ...
 function write(){
 	document.getElementsByClassName("pie").innerHTML = console.log("Hello World");
 }
 var PieImage = document.getElementsByTagName("img")[0]


 PieImage.addEventListener("click", log);
 function log(){
 	console.log("Hmmmm delicious pie")
}

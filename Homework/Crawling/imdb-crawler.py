...
def scrape_movie_page(dom):
    ...
    writers = ""
    actors = ""
    header = dom.by_class("header")[0]              #use elements to find the necessary information
    title = header.by_class("itemprop")[0].content
    infobar = dom.by_class("infobar")
    duration = infobar[0].by_tag("time")[0].content[:-5].strip()
    for i in range (0, len(infobar[0].by_tag("a"))-1,1):    # loop because there might be multiple genres
        genreTag = infobar[0].by_tag("a")[i]
        genre = genreTag.by_tag("span")[0].content
        genres += genre
        genres += "; "
    genres = genres[: -2]    
    txtblock = dom.by_class("txt-block")
    for i in range (0, len(txtblock[0].by_tag("span")),1):  # loop because there might be multiple directors
        director = txtblock[0].by_tag("span")[i].content
        directors += director
        directors += "; "
    directors = directors[: -2]
    for i in range (0, len(txtblock[1].by_tag("span")),1):  # loop because there might be multiple writers
        writer = txtblock[1].by_tag("span")[i].content
        writers += writer
        writers += "; "
    writers = writers[: -2]
    for i in range (0, len(txtblock[2].by_tag("span"))-1,1): # loop because there might be multiple actors
        actor = txtblock[2].by_tag("span")[i].content
        actors += actor
        actors += "; "
    actors = actors[: -5]
    ratingsbox = dom.by_class("star-box giga-star")[0]
    rating = ratingsbox.by_tag("div")[0].content.strip()
    starboxdetails = ratingsbox.by_class("star-box-details")[0]
    aValueNumberOfRatings = starboxdetails.by_tag("a")[0]
    NumberOfRatings = aValueNumberOfRatings.by_tag("span")[0].content
   
    
    

    # year = year.attr("a").content
    # hoe werkt die element library?
    # waarom zegt hij altijd dat 'list' object has no attribute 'by_id'
    # in hoeverre moet je specificeren waar de variabele zit?
